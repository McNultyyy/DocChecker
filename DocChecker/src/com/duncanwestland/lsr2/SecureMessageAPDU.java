...
import com.duncanwestland.chk.ReaderStartUpActivity;
...
public class SecureMessageAPDU {
	...
	byte[] DO99;
	byte[] DO8E;
	String DEBUG = ReaderStartUpActivity.DEBUG;
	/**
	 ...
	 */
	public byte[] assemble(byte[] apdu,APDU.Mode mode) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException{
		// assign convenient descriptors to the apdu bytes
		//Crypto.logger("apdu", apdu);
		byte CLA = apdu[0];
		byte INS = apdu[1];
		byte P1 = apdu[2];
		byte P2 = apdu[3];
		...
		if (mode == APDU.Mode.SELECT) {
			//LC = apdu[4];
			//if (LC!=0x02) {Log.e(DEBUG,"command length is not 2"); System.exit(LC);}
		...
		if (mode==APDU.Mode.SELECT) {  //create a DO87 data object
			...
			if (d.length>128) { 
				//Log.e(DEBUG,"d is too big to fit in a byte");
	...
	public boolean check(byte[] resp) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		...
		if (DO8E.length != CC.length){
			//Log.e(DEBUG,"DO8E and CC are different lengths");
			Crypto.logger("CC", CC);
		...
		for (int i=0;i<DO8E.length;i++){
			if (DO8E[i]!=CC[i]){
				//Log.e(DEBUG,"crypto checksum failed");
				return false;
			}
		}
		return true;
